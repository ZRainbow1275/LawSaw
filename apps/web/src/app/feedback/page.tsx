...
import { MessageSquarePlus, Rss, Bug, Lightbulb, HelpCircle, Send, Clock, CheckCircle2, XCircle, Loader2, Sparkles } from "lucide-react";
...
const feedbackTypes = [
  { value: "source_suggestion" as const, label: "信息源建议", icon: Rss, color: "text-blue-500 bg-blue-50" },
  { value: "bug_report" as const, label: "问题反馈", icon: Bug, color: "text-red-500 bg-red-50" },
  { value: "feature_request" as const, label: "功能建议", icon: Lightbulb, color: "text-amber-500 bg-amber-50" },
  { value: "other" as const, label: "其他", icon: HelpCircle, color: "text-neutral-500 bg-neutral-50" },
];

const statusConfig: Record<Feedback["status"], { label: string; color: string; icon: typeof Clock }> = {
  pending: { label: "待处理", color: "bg-neutral-100 text-neutral-600", icon: Clock },
  reviewing: { label: "处理中", color: "bg-blue-100 text-blue-600", icon: Loader2 },
  resolved: { label: "已解决", color: "bg-green-100 text-green-600", icon: CheckCircle2 },
  rejected: { label: "已关闭", color: "bg-red-100 text-red-600", icon: XCircle },
};
...
export default function FeedbackPage() {
  ...
  const [selectedType, setSelectedType] = useState<CreateFeedbackInput["type"] | null>(null);
  const [formData, setFormData] = useState({ title: "", content: "", contact_email: "", source_url: "", source_name: "" });
  const [submitted, setSubmitted] = useState(false);

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!selectedType || !formData.title || !formData.content) return;
    const payload: CreateFeedbackInput = { type: selectedType, title: formData.title, content: formData.content };
    ...
    createFeedback.mutate(payload, {
      onSuccess: () => {
        setSubmitted(true);
        setFormData({ title: "", content: "", contact_email: "", source_url: "", source_name: "" });
          ...
          >
            ...
            <motion.div className="mb-8 relative" variants={itemVariants}>
              ...
              <div className="flex items-center gap-3">
                <h1 className="text-2xl font-bold text-neutral-900">留言反馈</h1>
                <motion.div
              ...
              <p className="mt-1 text-sm text-neutral-500">向我们提交建议、反馈问题或推荐新的信息源</p>
            </motion.div>
              ...
              <motion.div className="lg:col-span-2" variants={itemVariants}>
                <Card>
                  <CardHeader className="bg-gradient-to-r from-primary-50/50 to-transparent border-b border-primary-100/30">
                    ...
                    <CardDescription>选择反馈类型并填写详细信息</CardDescription>
                  </CardHeader>
                  <CardContent className="pt-6">
                    <AnimatePresence mode="wait">
                      {submitted ? (
                        ...
                        >
                          <div>
                            <label className="mb-3 block text-sm font-medium text-neutral-700">反馈类型</label>
                            <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
                              {feedbackTypes.map((type, index) => (
                                ...
                                >
                                  ...
                                  <span className="relative z-10 text-xs font-medium text-neutral-700">{type.label}</span>
                                </motion.button>
                              ))}
                            </div>
                          </div>
                            ...
                            {selectedType && (
                              ...
                              >
                                {selectedType === "source_suggestion" && (
                                  ...
                                  >
                                    <div>
                                      <label className="mb-1 block text-sm font-medium text-neutral-700">信息源名称</label>
                                      <Input
                                    ...
                                    <div>
                                      <label className="mb-1 block text-sm font-medium text-neutral-700">信息源地址</label>
                                      <Input
                                        placeholder="https://..."
                                ...
                                >
                                  <label className="mb-1 block text-sm font-medium text-neutral-700">联系邮箱（选填）</label>
                                  <Input
                                    type="email"
                                ...
                                >
                                  ...
                                  <Button type="submit" disabled={createFeedback.isPending || !formData.title || !formData.content}>
                  ...
                  <CardContent>
                    {feedbacksLoading ? (
                      ...
                      >
                        ...
                        <p className="mt-3 text-sm text-neutral-500">暂无反馈记录</p>
                        <p className="mt-1 text-xs text-neutral-400">提交您的第一条反馈吧</p>
                      </motion.div>
                    ) : (
                      <div className="space-y-3">
                          ...
                          return (
                            ...
                            >
                              <div className="mb-2 flex items-start justify-between">
                                <h4 className="text-sm font-medium text-neutral-900 line-clamp-1">{feedback.title}</h4>
                                <Badge className={"ml-2 shrink-0 " + status.color}>
                                  <StatusIcon className="mr-1 h-3 w-3" />
                                  {status.label}
                                </Badge>
                              </div>
                              <p className="text-xs text-neutral-500">{formatTime(feedback.created_at)}</p>
                              {feedback.admin_response && (
                                ...
                                >
                                  <p className="text-xs text-neutral-600">
                                    <span className="font-medium text-primary-600">回复：</span>
                                    {feedback.admin_response}
                                  </p>
                                </motion.div>
                              )}
