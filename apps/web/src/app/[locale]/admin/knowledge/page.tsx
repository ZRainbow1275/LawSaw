...
function AdminKnowledgeContent() {
	...
	const [duplicateThresholdInput, setDuplicateThresholdInput] =
		useState("0.85");
	const [minCooccurrenceInput, setMinCooccurrenceInput] = useState("2");
	const [activeEntityId, setActiveEntityId] = useState<string | null>(null);
	const entityIdParam = searchParams.get("entityId");

	const duplicateThreshold = Number.isFinite(
		Number.parseFloat(duplicateThresholdInput),
	)
		? Math.min(0.99, Math.max(0.5, Number.parseFloat(duplicateThresholdInput)))
		: 0.85;
	const minCooccurrence = Number.isFinite(
		Number.parseInt(minCooccurrenceInput, 10),
	)
	...
	useEffect(() => {
		...
		setActiveEntityId(entityIdParam);
	}, [entityIdParam]);

	const closeEntityDrawer = () => {
		setActiveEntityId(null);
		clearSearchParams(["entityId"]);
	};

	const statsQuery = useKnowledgeGraphStats(isAdmin);
	const duplicatesQuery = useKnowledgeDuplicateCandidates(
		duplicateThreshold,
		12,
		isAdmin,
	);
	const centralityQuery = useKnowledgeDegreeCentrality(10, isAdmin);
	const cooccurrenceQuery = useKnowledgeCooccurrenceNetwork(
		minCooccurrence,
		10,
		isAdmin,
	);
	...
	const searchError =
		searchMode === "semantic"
			? semanticSearchQuery.error
			: hybridSearchQuery.error;
	const searchHasError =
		searchMode === "semantic"
			? semanticSearchQuery.isError
			: hybridSearchQuery.isError;

	const embeddingCoverage = useMemo(() => {
		if (!statsQuery.data || statsQuery.data.entity_count === 0) return 0;
		return (
			statsQuery.data.entities_with_embedding / statsQuery.data.entity_count
		);
	}, [statsQuery.data]);
		...
		mergeEntities.mutate(
			{ target_id: targetId, source_id: sourceId },
			{
				onSuccess: () => {
					success(
						t("Entities merged"),
						t("The duplicate queue and graph metrics have been refreshed."),
					);
				},
				onError: (cause) => {
					error(
			...
			<div className="space-y-6">
				<Card style={knowledgeAdminHeroStyle}>
					<CardHeader>
						...
						<CardDescription>
							{t(
								"Operate graph quality, review duplicates, run retrieval checks, and monitor embedding coverage from one console.",
							)}
						</CardDescription>
					</CardHeader>
				</Card>
						...
						<div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_minmax(0,1fr)]">
								...
								<CardHeader>
									<div className="flex items-center justify-between gap-3">
										<div>
											...
											<CardDescription>
												{t(
													"Run live semantic and hybrid retrieval checks against the tenant graph.",
												)}
											</CardDescription>
										</div>
										<Button
										...
										>
											<RefreshCw
												aria-hidden="true"
												className={
													searchLoading ? "h-4 w-4 animate-spin" : "h-4 w-4"
												}
											/>
											{t("Refresh")}
										</Button>
								...
								<CardContent className="space-y-4">
									<div className="grid gap-3 md:grid-cols-[minmax(0,1fr)_180px]">
										<Input
											...
											placeholder={t(
												"Search entities by policy concept, organization, or legal term",
											)}
									...
									{query.trim().length === 0 ? (
										<EmptyState
											title={t("Enter a search query")}
											description={t(
												"Results appear after you provide a real query against the knowledge graph.",
											)}
											className="py-8"
											icon={ScanSearch}
										/>
										...
										<p className="text-sm" style={knowledgeAdminMutedTextStyle}>
											{t("Searching graph")}
										</p>
									) : searchResults.length === 0 ? (
										<EmptyState
											title={t("No graph matches")}
											description={t(
												"Try another legal concept, organization alias, or policy term.",
											)}
											className="py-8"
										/>
									) : (
													...
													<div className="flex flex-wrap items-start justify-between gap-3">
														...
														<div className="flex flex-wrap items-center gap-2">
															<Badge variant="outline">
																{t("Mentions")}: {item.mention_count}
															</Badge>
															{"similarity" in item ? (
																<Badge variant="info">
																	{t("Similarity")}:{" "}
																	{formatPercent(Number(item.similarity))}
																</Badge>
															) : null}
														</div>
													</div>
							...
							<Card>
								<CardHeader>
									...
									<CardDescription>
										{t(
											"Monitor which entity classes dominate the tenant graph right now.",
										)}
									</CardDescription>
								</CardHeader>
								<CardContent className="space-y-3">
									{statsQuery.isLoading ? (
										<p className="text-sm" style={knowledgeAdminMutedTextStyle}>
											{t("Loading graph statistics")}
										</p>
									) : statsQuery.isError ? (
										<EmptyState
											variant="error"
											title={t("Failed to load graph statistics")}
											description={
										...
										<EmptyState
											title={t("No entity types yet")}
											description={t(
												"Ingest more real articles to build graph distribution signals.",
											)}
											className="py-8"
										/>
									) : (
												...
												<div className="flex items-center justify-between gap-3">
													<div>
														...
														<p
															...
															style={knowledgeAdminMutedTextStyle}
														>
															{t("Real entity count by ontology bucket")}
														</p>
													</div>
							...
							<Card>
								<CardHeader>
									<div className="flex items-center justify-between gap-3">
										<div>
											...
											<CardDescription>
												{t(
													"Review graph duplicate pairs generated from real similarity scoring and merge them deliberately.",
												)}
											</CardDescription>
										</div>
										<div className="w-32">
											<Input
												...
												onChange={(event) =>
													setDuplicateThresholdInput(event.target.value)
												}
												inputMode="decimal"
												placeholder="0.85"
												style={knowledgeAdminFieldSurfaceStyle}
								...
								<CardContent className="space-y-3">
									{duplicatesQuery.isLoading ? (
										<p className="text-sm" style={knowledgeAdminMutedTextStyle}>
											{t("Loading duplicate candidates")}
										</p>
									) : duplicatesQuery.isError ? (
										<EmptyState
											variant="error"
											title={t("Failed to load duplicate candidates")}
											description={
												duplicatesQuery.error instanceof Error
													? duplicatesQuery.error.message
													: t("Unknown error")
											}
											className="py-8"
										/>
										...
										<EmptyState
											title={t("No duplicate candidates")}
											description={t(
												"No duplicate pair exceeds the current similarity threshold.",
											)}
											className="py-8"
										/>
									) : (
										duplicatesQuery.data?.map((item) => (
											<div
												key={`${item.entity1.id}:${item.entity2.id}`}
												className="rounded-2xl border px-4 py-4"
												style={knowledgeAdminPanelStyle}
											>
												<div className="flex flex-wrap items-start justify-between gap-3">
													<div>
														...
														<p
															...
															style={knowledgeAdminMutedTextStyle}
														>
															{formatEntityTypeLabel(item.entity1.entity_type)}{" "}
															· {t("Mentions")}: {item.entity1.mention_count}
														</p>
													</div>
													<Badge variant="warning">
														{t("Similarity")}: {formatPercent(item.similarity)}
													</Badge>
												</div>
												<div
													className="mt-3 rounded-2xl border border-dashed px-4 py-3"
													style={knowledgeAdminNestedSurfaceStyle}
												>
													<p
													...
													<p
														...
														style={knowledgeAdminMutedTextStyle}
													>
														{formatEntityTypeLabel(item.entity2.entity_type)} ·{" "}
														{t("Mentions")}: {item.entity2.mention_count}
													</p>
												</div>
												<div className="mt-4 flex flex-wrap gap-2">
													<Button
														...
														variant="outline"
														onClick={() =>
															handleMerge(item.entity1.id, item.entity2.id)
														}
													...
													<Button
														...
														variant="outline"
														onClick={() =>
															handleMerge(item.entity2.id, item.entity1.id)
														}
														disabled={mergeEntities.isPending}
													>
														<GitMerge aria-hidden="true" className="h-4 w-4" />
							...
							<div className="space-y-4">
								<Card>
									<CardHeader>
										...
										<CardDescription>
											{t(
												"Watch the most connected entities to identify dominant policy actors and hubs.",
											)}
										</CardDescription>
									</CardHeader>
									<CardContent className="space-y-3">
										{centralityQuery.isLoading ? (
											<p
												...
												style={knowledgeAdminMutedTextStyle}
											>
												{t("Loading centrality")}
											</p>
										) : centralityQuery.isError ? (
											<EmptyState
												variant="error"
												title={t("Failed to load centrality")}
												description={
													centralityQuery.error instanceof Error
														? centralityQuery.error.message
														: t("Unknown error")
												}
												className="py-8"
											/>
											...
											<EmptyState
												title={t("No ranked entities yet")}
												description={t(
													"Graph analytics will appear after more real relations are ingested.",
												)}
												className="py-8"
											/>
										) : (
											centralityQuery.data?.map((item, index) => (
												<button
													type="button"
													key={item.entity.id}
													onClick={() => setActiveEntityId(item.entity.id)}
													className="block w-full rounded-2xl border px-4 py-4 text-left transition-colors hover:bg-[var(--control-hover-bg)]"
													style={knowledgeAdminPanelStyle}
												>
													<div className="flex items-start justify-between gap-3">
														<div>
															...
															<p
																...
																style={knowledgeAdminMutedTextStyle}
															>
																{formatEntityTypeLabel(item.entity.entity_type)}
															</p>
														</div>
														<Badge variant="outline">
															{t("Total degree")}: {item.total_degree}
														</Badge>
													</div>
													<p
														...
														style={knowledgeAdminMutedTextStyle}
													>
														{t("Outgoing degree")}: {item.out_degree} ·{" "}
														{t("Incoming degree")}: {item.in_degree}
													</p>
												</button>
											))
										)}
									...
									<CardHeader>
										<div className="flex items-center justify-between gap-3">
											<div>
												...
												<CardDescription>
													{t(
														"Inspect strong article-level co-occurrence edges that may deserve ontology review.",
													)}
												</CardDescription>
											</div>
											<div className="w-28">
												<Input
													...
													onChange={(event) =>
														setMinCooccurrenceInput(event.target.value)
													}
													inputMode="numeric"
													placeholder="2"
													style={knowledgeAdminFieldSurfaceStyle}
									...
									<CardContent className="space-y-3">
										{cooccurrenceQuery.isLoading ? (
											<p
												...
												style={knowledgeAdminMutedTextStyle}
											>
												{t("Loading co-occurrence edges")}
											</p>
										) : cooccurrenceQuery.isError ? (
											<EmptyState
												variant="error"
												title={t("Failed to load co-occurrence edges")}
												description={
													cooccurrenceQuery.error instanceof Error
														? cooccurrenceQuery.error.message
														: t("Unknown error")
												}
												className="py-8"
											/>
											...
											<EmptyState
												title={t("No co-occurrence edges yet")}
												description={t(
													"Lower the threshold or ingest more real graph links.",
												)}
												className="py-8"
											/>
										) : (
											cooccurrenceQuery.data?.map((item) => (
												<div
													key={`${item.entity1_id}:${item.entity2_id}`}
													className="rounded-2xl border px-4 py-4"
													style={knowledgeAdminPanelStyle}
												>
													<div className="flex items-center justify-between gap-3">
														...
														<Badge variant="info">
															{t("Co-occurrences")}: {item.cooccurrence_count}
														</Badge>
													</div>
												</div>
											))
