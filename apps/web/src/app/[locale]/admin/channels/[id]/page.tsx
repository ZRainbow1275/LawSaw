...
export default function AdminChannelDetailPage() {
								...
								>
									<div className="flex flex-wrap items-center justify-between gap-2">
										...
										<div className="flex flex-wrap gap-1">
											{policy.can_read ? (
												<Badge variant="secondary">{t("Can read")}</Badge>
											) : null}
	...
	const meta = (
		<MetaList
			title={t("Channel metadata")}
			icon={<Tag aria-hidden="true" className="h-4 w-4" />}
			items={[
				{
					label: t("Channel ID"),
					value: (
						<code className="break-all font-mono text-xs">{channel.id}</code>
					),
				},
				{
					label: t("Slug"),
					value: <code className="font-mono text-xs">{channel.slug}</code>,
				},
				{
					label: t("Created at"),
					value: formatDateTime(locale, channel.created_at, dateOptions),
				},
				{
					label: t("Updated at"),
					value: formatDateTime(locale, channel.updated_at, dateOptions),
				},
			]}
		/>
	);
