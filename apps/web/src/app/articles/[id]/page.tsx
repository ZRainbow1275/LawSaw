...
function sourceVisibilityDescription(
	...
	switch (visibility) {
		case "full":
			return t("This reader can inspect full source metadata, original URL, and operational health.");
		case "summary":
			return t("This reader can see the source label, but operational metadata and original URL remain hidden.");
		default:
			return t("This article can be read, but its source metadata is restricted by policy.");
	}
}
...
export default function ArticleDetailPage() {
	...
	const writeDecision = useAuthzDecision("article", articleId, "articles:write");
	const publishDecision = useAuthzDecision("article", articleId, "articles:publish");
	...
	const readerTocItems = useMemo(() => {
		const items: TOCItem[] = [
			...
			{ id: "article-source-section", text: t("Source visibility"), level: 1 as const },
		];
	...
	const { items: tocItems, activeId, setActiveId } = useTableOfContents(
		contentRef,
		readerTocItems,
	...
	const fallbackSourceLink = article.link && article.link.length > 0 ? article.link : null;
	const sourceView = article.source_view ?? null;
	const sourceVisibility = sourceView?.visibility ?? (fallbackSourceLink ? "full" : "hidden");
	const sourceViewUrl = sourceView?.original_url ?? (sourceView == null ? fallbackSourceLink : null);
	const sourceLabel = sourceView?.source_name || sourceView?.source_ref || t("Unknown source");
	...
	const publishedAtLabel = formatArticlePublishedAtLabel(locale, publishedAtDate);
	...
	return (
		<ReaderLayout>
			...
			<div className="min-h-screen transition-colors duration-300" style={readerTone.scopeStyle}>
				<nav className="sticky top-0 z-40 border-b backdrop-blur-sm" style={readerTone.navStyle}>
				...
				<div className="relative mx-auto max-w-4xl">
					...
					<article className="mx-auto max-w-2xl px-5 pb-24 lg:pb-20" style={readingStyles}>
						<header
						...
						>
							<motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
								...
								<div className="mb-4 flex items-center gap-2 text-sm" style={readerTone.subtleStyle}>
									{article.author ? (
										<span style={readerTone.strongTextStyle}>{article.author}</span>
									) : null}
								...
								{article.summary && (
									<p className="mt-6 text-lg leading-relaxed" style={readerTone.mutedStyle}>
										{article.summary}
									</p>
								)}
								...
								>
									<div className="flex flex-wrap items-center gap-2">
										<span
											className={readerInlineChipClassName}
											style={canWriteArticle ? readerAccentChipStyle : readerMutedChipStyle}
										>
									...
									<p className="mt-3 text-sm" style={readerTone.mutedStyle}>
										{canWriteArticle || canPublishArticle
											? t("Use the controls below to continue the article workflow.")
											: t(
								...
								>
									...
									<div className="mt-4 grid gap-3 text-sm sm:grid-cols-2">
										<div className="rounded-2xl px-4 py-3 shadow-sm" style={readerTone.metaCardStyle}>
											<p className="text-xs uppercase tracking-wide" style={readerTone.subtleStyle}>{t("Source label")}</p>
											<p className="mt-1 font-medium" style={readerTone.bodyStyle}>{sourceLabel}</p>
										</div>
										<div className="rounded-2xl px-4 py-3 shadow-sm" style={readerTone.metaCardStyle}>
											<p className="text-xs uppercase tracking-wide" style={readerTone.subtleStyle}>{t("Source type")}</p>
											<p className="mt-1 font-medium" style={readerTone.bodyStyle}>
										...
										{sourceView?.health_status ? (
											<div className="rounded-2xl px-4 py-3 shadow-sm" style={readerTone.metaCardStyle}>
												<p className="text-xs uppercase tracking-wide" style={readerTone.subtleStyle}>{t("Health status")}</p>
												<p className="mt-1 font-medium" style={readerTone.bodyStyle}>
													{t(sourceHealthStatusLabelKey(sourceView.health_status))}
												</p>
											</div>
										) : null}
										{sourceView?.schedule ? (
											<div className="rounded-2xl px-4 py-3 shadow-sm" style={readerTone.metaCardStyle}>
												<p className="text-xs uppercase tracking-wide" style={readerTone.subtleStyle}>{t("Refresh schedule")}</p>
												<p className="mt-1 font-medium" style={readerTone.bodyStyle}>{sourceView.schedule}</p>
											</div>
										) : null}
										{sourceView?.source_ref ? (
											...
											>
												<p className="text-xs uppercase tracking-wide" style={readerTone.subtleStyle}>{t("Source reference")}</p>
												<p className="mt-1 font-medium" style={readerTone.bodyStyle}>{sourceView.source_ref}</p>
											</div>
										) : null}
									</div>

									{sourcePolicy ? (
										<div className="mt-4 flex flex-wrap gap-2 text-xs" style={readerTone.mutedStyle}>
											<span className={readerInlineChipClassName} style={readerMutedChipStyle}>
												{t("Role tier")}: {sourcePolicyRoleTierLabel(t, sourcePolicy.role_tier)}
											</span>
											{sourcePolicy.matched_relation ? (
												<span className={readerInlineChipClassName} style={readerMutedChipStyle}>
													{t("Matched relation")}: {sourcePolicy.matched_relation}
												</span>
											) : null}
						...
						<footer className="mt-16 border-t border-current/10 pt-8">
							<div className="flex items-center justify-between text-sm">
								<button
									type="button"
									onClick={() => router.back()}
									className={cn("transition-colors", readerTone.actionHoverClassName)}
									style={readerTone.actionStyle}
								>
									← {t("Back to list")}
								</button>
