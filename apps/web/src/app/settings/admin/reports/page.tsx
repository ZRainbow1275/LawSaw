...
function reportStatusVariant(status: ReportStatus): "outline" | "warning" | "success" | "secondary" | "destructive" {
	switch (status) {
		case "published":
			return "success";
		case "approved":
...
function AdminReportsContent() {
	...
	const filteredTemplatesQuery = useReportTemplates(periodFilter === "all" ? undefined : periodFilter);
	...
	useEffect(() => {
		if (selectedTemplate) {
			setDraft({
				...
				period_type: (selectedTemplate.period_type as ReportPeriodType) || DEFAULT_PERIOD_TYPE,
	...
	const summaryCards = [
		{
			...
			description: t("Real report template definitions available to this tenant."),
			icon: FileCode2,
		},
		{
		...
		},
		{
			...
			description: t("Live report records fetched from the current tenant workspace."),
			icon: ClipboardList,
		},
	];
	...
	const handleSubmitTemplate = () => {
		...
		if (!selectedTemplate) {
			createTemplate.mutate(payload, {
				onSuccess: (created) => {
					success(t("Template created"), t("The report template is ready for operational use."));
					setSelectedTemplateId(created.id);
				},
				onError: (cause) => {
					error(t("Create failed"), cause instanceof Error ? cause.message : t("Unknown error"));
				},
			});
			return;
		}
		...
		updateTemplate.mutate(
			...
			{
				onSuccess: () => {
					success(t("Template updated"), t("Template changes are now visible to report operators."));
				},
				onError: (cause) => {
					error(t("Update failed"), cause instanceof Error ? cause.message : t("Unknown error"));
				},
			},
		);
	};
	...
	const confirmDeleteTemplate = () => {
		...
		deleteTemplate.mutate(selectedTemplate.id, {
			onSuccess: () => {
				success(t("Template archived"), t("The template was soft deleted and removed from active operations."));
				setConfirmArchiveOpen(false);
				handleResetDraft();
			},
			onError: (cause) => {
				error(t("Delete failed"), cause instanceof Error ? cause.message : t("Unknown error"));
			},
		});
	};

	const templateBusy = createTemplate.isPending || updateTemplate.isPending || deleteTemplate.isPending;
	...
	return (
		...
		<div className="min-h-screen" style={{ backgroundColor: "var(--color-background)" }}>
				...
				<div className="space-y-6 p-4 md:p-6">
					<Card>
						<CardHeader>
							...
							<CardDescription>
								{t("Govern report templates, monitor lifecycle status, and keep delivery policy aligned with tenant roles.")}
							</CardDescription>
						</CardHeader>
					</Card>
										...
										<Card key={item.title}>
											<CardContent className="flex items-start gap-4 p-6">
												...
												<div className="space-y-1">
													<p className="text-sm" style={mutedTextStyle}>{item.title}</p>
													<p className="text-3xl font-semibold" style={headingStyle}>{item.value}</p>
													<p className="text-xs" style={mutedTextStyle}>{item.description}</p>
												</div>
											</CardContent>
										</Card>
									);
							...
							<div className="grid gap-4 xl:grid-cols-[minmax(0,0.95fr)_minmax(0,1.05fr)]">
								<Card>
									<CardHeader>
										...
										<CardDescription>
											{t("Use real generation status and period filters to focus the current delivery queue.")}
										</CardDescription>
									</CardHeader>
									<CardContent className="space-y-4">
										<div className="grid gap-3 md:grid-cols-2">
											<div>
												<label htmlFor="report-period-filter" className="mb-1 block text-xs font-medium uppercase tracking-wide" style={mutedTextStyle}>
													{t("Period filter")}
												</label>
												<select
													id="report-period-filter"
													value={periodFilter}
													onChange={(event) => setPeriodFilter(event.target.value as "all" | ReportPeriodType)}
											...
											<div>
												<label htmlFor="report-status-filter" className="mb-1 block text-xs font-medium uppercase tracking-wide" style={mutedTextStyle}>
													{t("Status filter")}
												</label>
												<select
											...
											className="h-10 w-full rounded-lg border px-3 text-sm outline-none focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[color:var(--color-primary-300)]"
												...
												>
													...
													{REPORT_STATUSES.map((value) => (
														<option key={value} value={value}>{reportStatusLabel(t, value)}</option>
													))}
												</select>
											</div>
										</div>
										...
										<div className="grid gap-3 md:grid-cols-2">
											{REPORT_STATUSES.map((status) => (
												<div key={status} className="rounded-2xl border px-4 py-3" style={softSurfaceStyle}>
													<div className="flex items-center justify-between gap-3">
														<Badge variant={reportStatusVariant(status)}>{reportStatusLabel(t, status)}</Badge>
														<span className="text-lg font-semibold" style={headingStyle}>{statusCounts.get(status) ?? 0}</span>
													</div>
												</div>
											))}
										</div>
										...
										<p className="text-xs" style={mutedTextStyle}>
											{t("This view surfaces real report records and real template definitions. No mock data is used.")}
										</p>
									</CardContent>
								</Card>
								...
								<Card>
									<CardHeader>
										<div className="flex items-center justify-between gap-3">
											<div>
												...
												<CardDescription>
													{t("Track live reports, article volume, and publish readiness from the current filter context.")}
												</CardDescription>
											</div>
											<Button variant="outline" size="sm" onClick={() => recentReportsQuery.refetch()} disabled={recentReportsQuery.isFetching}>
												<RefreshCw aria-hidden="true" className={recentReportsQuery.isFetching ? "h-4 w-4 animate-spin" : "h-4 w-4"} />
												{t("Refresh")}
											</Button>
										</div>
									</CardHeader>
									<CardContent className="space-y-3">
										{recentReportsQuery.isLoading ? (
											<p className="text-sm" style={mutedTextStyle}>{t("Loading reports")}</p>
										) : recentReportsQuery.isError ? (
											<EmptyState
												variant="error"
												title={t("Failed to load reports")}
												description={recentReportsQuery.error instanceof Error ? recentReportsQuery.error.message : t("Unknown error")}
												action={{ label: t("Retry"), onClick: () => recentReportsQuery.refetch() }}
												className="py-8"
											/>
										) : (recentReportsQuery.data?.data.length ?? 0) === 0 ? (
											<EmptyState
												title={t("No reports matched")}
												description={t("Adjust the period or status filter to inspect another operational slice.")}
												className="py-8"
											/>
										) : (
											recentReportsQuery.data?.data.map((report) => (
												<div key={report.id} className="rounded-2xl border px-4 py-4" style={surfaceStyle}>
													<div className="flex flex-wrap items-start justify-between gap-3">
														<div className="min-w-0 space-y-1">
															<p className="truncate text-base font-semibold" style={headingStyle}>{report.title}</p>
															<p className="text-xs" style={mutedTextStyle}>{report.report_number}</p>
														</div>
														<div className="flex flex-wrap items-center gap-2">
															<Badge variant={reportStatusVariant(report.status)}>{reportStatusLabel(t, report.status)}</Badge>
															<Badge variant="outline">{periodLabel(t, report.period_type)}</Badge>
														</div>
													</div>
													<div className="mt-3 grid gap-3 text-sm md:grid-cols-3" style={mutedTextStyle}>
														<div>
															<p className="text-xs uppercase tracking-wide" style={mutedTextStyle}>{t("Report window")}</p>
															<p className="mt-1">{report.period_start} - {report.period_end}</p>
														</div>
														<div>
															<p className="text-xs uppercase tracking-wide" style={mutedTextStyle}>{t("Articles included")}</p>
															<p className="mt-1">{report.article_count}</p>
														</div>
														<div>
															<p className="text-xs uppercase tracking-wide" style={mutedTextStyle}>{t("Updated at")}</p>
															<p className="mt-1">{formatDateTime(locale, report.updated_at, { year: "numeric", month: "2-digit", day: "2-digit", hour: "2-digit", minute: "2-digit" })}</p>
														</div>
													</div>
												</div>
											))
								...
								<Card>
									<CardHeader>
										<div className="flex items-center justify-between gap-3">
											<div>
												...
												<CardDescription>
													{t("Filter real tenant templates by cadence, then open one for editing or create a new operational baseline.")}
												</CardDescription>
											</div>
									...
									<CardContent className="space-y-3">
										{filteredTemplatesQuery.isLoading ? (
											<p className="text-sm" style={mutedTextStyle}>{t("Loading templates")}</p>
										) : filteredTemplatesQuery.isError ? (
											<EmptyState
												variant="error"
												title={t("Failed to load templates")}
												description={filteredTemplatesQuery.error instanceof Error ? filteredTemplatesQuery.error.message : t("Unknown error")}
												action={{ label: t("Retry"), onClick: () => filteredTemplatesQuery.refetch() }}
												className="py-8"
											/>
										) : templates.length === 0 ? (
											<EmptyState
												title={t("No templates matched")}
												description={t("Create the first template for this cadence or switch to another period filter.")}
												className="py-8"
											/>
										) : (
											templates.map((template) => {
												...
												return (
													<button
														...
														style={isSelected ? selectedSurfaceStyle : surfaceStyle}
													>
														<div className="flex items-start justify-between gap-3">
															<div className="min-w-0">
																<p className="truncate text-base font-semibold" style={headingStyle}>{template.name}</p>
																<p className="mt-1 text-xs" style={mutedTextStyle}>{template.description || t("No template description yet.")}</p>
															</div>
															<div className="flex flex-wrap items-center gap-2">
																<Badge variant="outline">{periodLabel(t, template.period_type)}</Badge>
																{template.is_builtin ? <Badge variant="secondary">{t("Built-in")}</Badge> : null}
																<Badge variant={template.is_active ? "success" : "outline"}>{template.is_active ? t("Active") : t("Disabled")}</Badge>
															</div>
														</div>
														<p className="mt-3 text-sm leading-6" style={mutedTextStyle}>{templateSummary(template) || t("No template body yet.")}</p>
														<p className="mt-3 text-xs" style={mutedTextStyle}>{t("Updated at")}: {formatDateTime(locale, template.updated_at, { year: "numeric", month: "2-digit", day: "2-digit", hour: "2-digit", minute: "2-digit" })}</p>
													</button>
												);
											})
										)}
			...
			<ConfirmActionModal
				...
				description={t("Custom templates can be updated live and soft deleted without losing audit history.")}
