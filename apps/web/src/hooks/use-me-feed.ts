...
import { type Article, assertArticle } from "@/lib/api/types";
...
export interface MeFeedResponse {
	...
	/** Backend serves banners separately at `/api/v1/banners/active`; optional here for legacy callers. */
...
function assertChannelRecord(
	value: unknown,
	path = "channel",
): asserts value is FeedChannelRecord {
	if (
		!isRecord(value) ||
...
function assertBannerRecord(
	value: unknown,
	path = "banner",
): asserts value is BannerRecord {
	if (
		!isRecord(value) ||
		typeof value.id !== "string" ||
...
function assertArticlePinRecord(
	value: unknown,
	path = "articlePin",
...
function assertMeFeedResponse(
	value: unknown,
	path = "meFeed",
): asserts value is MeFeedResponse {
	if (!isRecord(value)) {
	...
	if (
		!Array.isArray(value.visible_channels) ||
		!Array.isArray(value.pinned_articles) ||
		!Array.isArray(value.articles)
	) {
		throw new Error(`${path} collections are invalid`);
	}
	if (value.banners !== undefined && !Array.isArray(value.banners)) {
		throw new Error(`${path}.banners must be an array when present`);
	}
	for (const [index, item] of value.visible_channels.entries()) {
		assertChannelRecord(item, `${path}.visible_channels[${index}]`);
	}
	if (Array.isArray(value.banners)) {
