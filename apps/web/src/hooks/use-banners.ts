...
function assertBannerList(
	value: unknown,
	path = "banners",
): asserts value is BannerRecord[] {
	if (!Array.isArray(value)) throw new Error(`${path} must be an array`);
	for (const [index, item] of value.entries())
		assertBannerRecord(item, `${path}[${index}]`);
}
...
export function useActiveBanners(channelIds: string[]) {
	...
	return useQuery({
		...
		queryFn: () =>
			apiClient.get<BannerRecord[]>(
				`/api/v1/banners/active${query}`,
				assertBannerList,
			),
		staleTime: 15_000,
	});
...
export function useAdminBanners(includeArchived = false) {
	return useQuery({
		...
		queryFn: () =>
			apiClient.get<BannerRecord[]>(
				`/api/v1/admin/banners?include_archived=${includeArchived}`,
				assertBannerList,
			),
		staleTime: 10_000,
	});
...
export function useUpdateBanner() {
	const queryClient = useQueryClient();
	return useMutation({
		mutationFn: ({ id, ...input }: UpdateBannerInput) =>
			apiClient.patch<BannerRecord>(
