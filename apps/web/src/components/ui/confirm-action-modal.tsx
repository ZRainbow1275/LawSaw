	...
	return (
		<Modal isOpen={isOpen} onClose={busy ? () => {} : onClose} size="sm">
			<ModalHeader className="pr-14">
				<h2 className="text-lg font-semibold" style={headingStyle}>{title}</h2>
			</ModalHeader>
			<ModalBody>
				{description ? (
					<p className="text-sm leading-6" style={mutedTextStyle}>{description}</p>
				) : null}
			...
			<ModalFooter className="justify-end">
				...
				<Button
					...
					onClick={onConfirm}
					disabled={busy}
					className={cn(confirmVariant === "outline" ? "[border-color:var(--color-border)]" : "")}
				>
