					...
					<Card>
						<CardContent className="p-4">
							<div className="flex items-center gap-3">
								...
								<div>
									<p className="text-2xl font-bold">{importanceData.total}</p>
									<p className="text-sm text-neutral-500">
										{t("Articles Assessed")}
									</p>
								</div>
				...
				<Card>
					<CardHeader>
						<CardTitle className="flex items-center gap-2">
							<Star className="h-5 w-5 text-primary-500" aria-hidden="true" />
					...
					<CardContent>
						{importanceError ? (
							...
							<EmptyState title={t("No importance data")} className="py-10" />
						) : (
				...
				<Card>
					...
					<CardContent>
						{authorityError ? (
							...
							<EmptyState title={t("No authority data")} className="py-10" />
						) : (
			...
			<Card>
				...
				<CardContent>
					{issuerError ? (
						...
						<EmptyState title={t("No issuer data")} className="py-10" />
					) : (
						<div>
							...
							<div className="grid grid-cols-1 gap-x-8 lg:grid-cols-2">
								<IssuerRanking items={issuerData.items.slice(0, 10)} />
								{issuerData.items.length > 10 && (
									<IssuerRanking items={issuerData.items.slice(10, 20)} />
								)}
							</div>
						</div>
					)}
