...
- [ ] [MTLS-301] 内部服务 mTLS：实现 gateway → api 的 mTLS（服务端校验客户端证书，客户端校验服务端证书），并给出“无证书无法访问”的可重复验证步骤 - `docker-compose.enterprise.yml`、`infra/caddy/*`、`infra/pki/*`、`scripts/enterprise/*`
- [ ] [SECRETS-301] 集中 KMS/Secrets：引入 Vault（非 dev 模式）并落地最小权限策略；API/worker 启动时从 Vault 拉取敏感配置（DB/Redis/AI key 等），docker-compose 不再明文持有核心 secrets；提供密钥轮换/吊销的演示步骤 - `docker-compose.enterprise.yml`、`crates/law-eye-common/src/config.rs`、`scripts/enterprise/vault-*`
- [ ] [TENANT-301] 多租户隔离（逻辑隔离 + RLS）：新增 tenants 表与 tenant_id 字段，启用 Postgres RLS 并在 API 每请求设置 tenant 上下文；Playwright 验证 A/B 两个租户用户数据完全隔离（文章/信息源/反馈/知识图谱至少覆盖其一） - `crates/law-eye-db/migrations/*`、`crates/law-eye-core/src/*`、`crates/law-eye-api/src/*`、`apps/web/src/*`
- [ ] [ENC-301] 敏感字段加密/脱敏：至少对 `feedbacks.content/contact_email` 或等价敏感字段实现“静态存储加密 + 运行时可解密展示”（密钥来自 Vault Transit/Envelope），并提供脱敏展示策略 - `crates/law-eye-core/src/feedback.rs`、`crates/law-eye-db/migrations/*`、`scripts/enterprise/vault-*`
- [ ] [AUDIT-301] 审计日志不可篡改：audit_logs 增加 append-only 约束（禁止 UPDATE/DELETE）+ hash 链（prev_hash/hash/seq），并提供验证脚本（随机抽检 hash 链一致性）；补齐关键操作审计覆盖（至少 Sources/Articles/Feedback） - `crates/law-eye-db/migrations/*`、`crates/law-eye-core/src/audit.rs`、`scripts/enterprise/audit-verify.*`
- [ ] [CI-301] 可靠发布（CI 门禁）：补齐 GitHub Actions CI（cargo test/clippy、pnpm test/build、依赖漏洞扫描、格式化检查），并固化可重复构建产物（SBOM/锁定文件） - `.github/workflows/*`、`Cargo.lock`、`apps/web/pnpm-lock.yaml`
- [ ] [OBJ-301] 对象存储 + 元数据表：引入 MinIO（S3）并落地 objects 元数据表；实现“用户头像上传 → 对象存储 → 生成可访问 URL → 前端展示”全链路（真实写入/读取，不用 mock） - `docker-compose.enterprise.yml`、`crates/law-eye-db/migrations/*`、`crates/law-eye-api/src/routes/*`、`apps/web/src/app/settings/*`
...
- [x] [FE-KG-002] 知识图谱画布交互缺陷（滚轮缩放失效/视图偶发重置/滚轮 preventDefault 报 passive 错误）✅ 修复 wheel 事件改为非 passive 监听；避免容器尺寸抖动导致初始化视图重复覆盖用户操作；`setPointerCapture` 增加兜底避免异常阻断交互 - `apps/web/src/components/knowledge/knowledge-canvas.tsx`
