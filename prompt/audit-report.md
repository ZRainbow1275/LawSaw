...
- [ ] [REL-C01] Worker 任务无超时（外部依赖/长任务必须可中断）- `crates/law-eye-worker/src/main.rs`
- [ ] [REL-C02] AI 429 无特殊处理（退避重试/预算/降级）- `crates/law-eye-worker/src/main.rs`、`crates/law-eye-ai/src/*`
- [ ] [REL-C03] 队列 BRPOPLPUSH + ZADD 非原子（任务可能卡死/丢失语义）- `crates/law-eye-queue/src/lib.rs`
- [ ] [REL-C04] 延迟队列处理非原子（到期任务回迁存在竞态）- `crates/law-eye-queue/src/lib.rs`
- [ ] [REL-C05] 无熔断器（关键出站依赖需要隔离/快速失败/半开探测）- `crates/law-eye-common/src/*`、`crates/law-eye-ai/src/*`
- [ ] [REL-C06] 无 CD 部署流程（自动发布/回滚/门禁）- `.github/workflows/*`、`docs/*`
- [ ] [PERF-C01] 前端未使用 Image 组件（图片优化缺失）- `apps/web/src/*`
- [ ] [PERF-C02] N+1 查询问题（随数据量指数放大）- `crates/law-eye-worker/src/main.rs`
- [ ] [PERF-C03] 文章逐条 INSERT（应批量写入/Upsert）- `crates/law-eye-worker/src/main.rs`
- [ ] [PERF-C04] 全量读取对象到内存（大文件 OOM 风险；应流式传输）- `crates/law-eye-core/src/object.rs`
- [ ] [DATA-C01] 批量操作无数量限制（DoS/锁表风险）- `crates/law-eye-api/src/routes/articles.rs`
- [ ] [DATA-C02] 状态同步/冲突解决缺失（ETag/If-Match/乐观并发/冲突 UI）- `apps/web/src/*`、`crates/law-eye-api/src/routes/*`、`crates/law-eye-db/migrations/*`
- [ ] [DATA-C03] 无软删除机制（需一致化全表查询/索引/约束/恢复策略）- `crates/law-eye-db/migrations/*`
- [ ] [DATA-C04] 补偿删除失败被忽略（对象存储/元数据一致性）- `crates/law-eye-core/src/object.rs`
- [ ] [DEP-C01] 缺少 cargo-audit（供应链门禁）- `.github/workflows/*`
- [ ] [I18N-C01] 国际化未实现（Locale 路由/文案/格式化/后端消息）- `apps/web/*`、`crates/law-eye-common/src/*`
