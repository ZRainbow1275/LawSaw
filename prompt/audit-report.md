...
**审计基线**：Git `fc3bc7b`（工作区存在未提交变更；详见第 7 节）  
...
### CRITICAL（必须修复）
- [ ] [OPS-101] 工作区存在大量未提交变更，需收敛为可追溯提交（否则审计不可复现）- `git status`（需分批整理）
- [ ] [SEC-101] API 启动日志泄露敏感信息（DB/Redis URL）- `crates/law-eye-api/src/main.rs`
- [ ] [SEC-102] 管理/写接口未强制鉴权与授权（Sources/Articles 批处理/发布归档等）- `crates/law-eye-api/src/routes/*`、`crates/law-eye-api/src/middleware/auth_guard.rs`
