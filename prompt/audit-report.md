...
- [ ] [AI-202] AI 状态接口返回不完整且“完成口径”错误 - `crates/law-eye-api/src/routes/ai.rs` 现状：`GET /api/v1/ai/status/{article_id}` 固定返回 `category: null`，并以 `article.status == \"ai_processed\"` 判断（该状态不在 DB 约束/前端枚举中）。方案：按 `articles.category_id` join `categories` 填充 `category`（建议返回 `categories.name` 或 `slug`）；“完成口径”改为 `ai_processed_at IS NOT NULL`（由 worker 写入）。
