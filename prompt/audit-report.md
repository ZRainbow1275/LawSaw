...
- [x] [SEC-102] 管理/写接口未强制鉴权与授权（Sources/Articles/AI/Search 等） ✅ `/api/v1/*` 默认鉴权（仅 `/api/v1/auth/*` 例外），并按 `roles.permissions` 做授权（`articles:read/articles:publish/articles:write/sources:read/categories:read/*`）- `crates/law-eye-api/src/routes/mod.rs`、`crates/law-eye-api/src/routes/{articles,categories,sources,ai,search}.rs`
- [ ] [SEC-103] 登录/注册缺少 Rate Limit（代码存在但未接入）- `crates/law-eye-api/src/middleware/rate_limit.rs`、`crates/law-eye-api/src/routes/auth.rs`
- [ ] [SEC-104] Cookie Session 模式缺少 CSRF 保护策略（除 SameSite 外）- `crates/law-eye-api/src/main.rs`（需新增/接入中间件）
- [ ] [API-101] 前后端契约不一致：前端调用 DELETE/过滤参数，但后端缺失 - `apps/web/src/hooks/use-articles.ts`、`crates/law-eye-api/src/routes/articles.rs`

### HIGH（高优先级）
- [ ] [DATA-101] 信息源 last_fetch/last_error 未更新（运维失真）- `crates/law-eye-worker/src/main.rs`、`crates/law-eye-core`/`crates/law-eye-db`
- [ ] [API-102] OpenAPI/Swagger 未实际暴露（虽有 utoipa 注解）- `crates/law-eye-api/src/*`
- [ ] [FE-101] 前端 auth store 将用户对象持久化到 localStorage（PII 风险）- `apps/web/src/stores/auth-store.ts`
- [ ] [BIZ-101] ArticleStats high_risk_count 为 TODO/固定 0（统计口径缺失）- `crates/law-eye-api/src/routes/articles.rs`
- [ ] [QUEUE-101] 队列任务缺少 DLQ/超时/退避重试/幂等键 - `crates/law-eye-queue/src/lib.rs`、`crates/law-eye-worker/src/main.rs`

### MEDIUM（中优先级）
- [ ] [PWA-101] 离线支持与 PWA（manifest + SW + 缓存策略）- `apps/web/src/app/*`、`apps/web/next.config.mjs`
- [ ] [CONSIST-101] 增加 API 契约校验（至少 schema 对齐/运行时断言）- `apps/web/src/lib/api/types.ts` + 后端 responses
...
**报告生成时间**：2026-01-24  
**版本**：v2.0（本报告将随 Ralph Loop 修复持续更新）
