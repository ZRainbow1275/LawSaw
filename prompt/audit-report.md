...
- [ ] [FE-SOURCE-401] Sources 添加信息源：当类型为 spider 时提供必填 selector 配置输入并提交真实 config（避免产生无效 spider 配置导致 worker 报错）- `apps/web/src/app/sources/page.tsx`
- [ ] [DEV-401] Next Dev：配置 `allowedDevOrigins` 兼容 `127.0.0.1` 访问，消除跨域告警并避免未来版本中断 - `apps/web/next.config.mjs`
