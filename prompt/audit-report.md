...
- [x] [V26-H022] **无 Range 请求支持** ✅ `/api/v1/objects/{id}` 支持 RFC7233 单 Range：206 Partial Content / 416 Range Not Satisfiable，并通过 S3 `GetObject(range=...)` 实现高效分段流式读取 - `crates/law-eye-api/src/routes/objects.rs`、`crates/law-eye-core/src/object.rs` (2.6audit.md:341)
- [x] [V26-H023] 无行级版本/乐观锁 - 并发更新无保护 ✅ 为 users/feedbacks 等核心表增加 `version` + bump trigger，并为写接口强制 `If-Match` / 返回 `ETag`（缺失 428、冲突 412）；前端更新用户资料/头像自动带 `If-Match` - `crates/law-eye-db/migrations/024_row_versions.sql`、`crates/law-eye-api/src/routes/{mod,users,feedbacks}.rs`、`crates/law-eye-core/src/{user,feedback,object}.rs`、`apps/web/src/app/settings/page.tsx` (2.6audit.md:350)
- [x] [V26-H024] **Root token 存文件** ✅ Vault root token 不再落盘：init 输出仅在内存中短暂存在，用于一次性 bootstrap（写入 `law-eye-bootstrap` policy + cert mapping）后立即 `token revoke -self`；同时清理遗留 `init.json/root.token` 文件；rotate/revoke 脚本改为 cert auth 登录获取短期 token - `scripts/enterprise/vault-init.sh`、`scripts/enterprise/vault-init-enterprise.sh`、`scripts/enterprise/vault-rotate.sh`、`scripts/enterprise/vault-revoke.sh` (2.6audit.md:375)
- [x] [V26-H025] **PostgreSQL 无 TDE** ✅ 引入 pg_tde（Percona PG17）实现数据库落盘加密：默认启用 `tde_heap` + `pg_tde.enforce_encryption=on`（新表默认加密）；本地使用 file provider（keyring file），企业模式使用 Vault KV v2 作为 KMS（`vault-init-enterprise.sh` 自动创建 `tde` mount + `law-eye-pg-tde` policy + token 文件供 Postgres 读取）- `Dockerfile.postgres-pgvector`、`docker-compose.yml`、`docker-compose.enterprise.yml`、`scripts/enterprise/vault-init-enterprise.sh`、`README.md`、`.env.example` (2.6audit.md:395)
