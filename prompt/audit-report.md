...
- [ ] [OPS-401] 无 DockerHub 环境可重复启动全栈（用于联调/E2E），且不重复构建镜像/不误杀进程 - `scripts/no-dockerhub/*`
- [ ] [E2E-401] 固化 Playwright E2E 测试套件（注册/登录/文章/搜索/信息源抓取等关键用户流），纳入门禁并提供可复现运行方式 - `apps/web/*`、`scripts/*`
