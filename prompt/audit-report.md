...
- [ ] [ANALYTICS-301] 统计分析页「分类统计」缺少“未分类”分桶且口径依赖前端聚合（可能与 total 不一致），需改用同一后端聚合接口（与 Dashboard 共用）- `apps/web/src/app/analytics/page.tsx`、`crates/law-eye-api/src/routes/articles.rs`
