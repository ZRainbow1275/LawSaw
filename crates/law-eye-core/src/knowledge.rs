...
impl KnowledgeService {
    ...
    pub async fn process_article(
        ...
        for extracted in &extraction.entities {
            if let Ok(entity_id) = self.upsert_entity(extracted).await {
                self.link_article_entity(article_id, entity_id, extracted)
                    .await?;
    ...
    async fn upsert_relation(
        ...
        let source_id: Option<(Uuid,)> =
            sqlx::query_as("SELECT id FROM entities WHERE name = $1 AND entity_type = $2")
                .bind(&source.name)
    ...
    pub async fn get_top_entities(&self, limit: i64) -> Result<Vec<Entity>> {
        let entities = sqlx::query_as::<_, Entity>(
